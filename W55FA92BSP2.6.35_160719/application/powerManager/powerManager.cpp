...
#define PWRMGR_DEV	"/sys/devices/platform/w55fa95-clk/clock"
#define ADC_DEV		"/sys/devices/platform/w55fa95-adc/adc"
...
extern unsigned long EDesEn_Crypt(unsigned long rdmdata);//加密对称算法
...
bool ProcessPowerState(unsigned int state)
{
	...
	else if (state == POWER_STATE_RTC_POWER_OFF) {
		printf("enter POWER_STATE_RTC_POWER_OFF\n");
		// this command should be done in desktopManager
#if 0
		system("killall -9 saplayer");
		sync();
		write(fd_pwrmgr, "rpo", sizeof("rpo"));
#endif
		while (1)
			sleep(1);
	}
	else if (state == POWER_STATE_POWER_OFF) {
		printf("enter POWER_STATE_POWER_OFF\n");
		// this command should be done in desktopManager
#if 0
		system("killall -9 saplayer");
		sync();
		write(fd_pwrmgr, "po", sizeof("po"));
#endif
		while (1)
			sleep(1);
	}
...
bool PM_Init(void)
{
	unsigned long rdmdata;//随机数,从内核中获取
...
if (ioctl(fd_sysmgr, SYSMGR_IOC_EdesEn_get_rdmdata, &rdmdata) < 0) {//获取随机数
		fprintf(stderr,"SYSMGR_IOC_EdesEn_crypt ioctl failed:%d\n", errno);
	}	

	printf("SYSMGR_IOC_get_rdmdata:%x\n",rdmdata);

	rdmdata = EDesEn_Crypt(rdmdata);//算出数据传入内核，与加密芯片匹配
	if (ioctl(fd_sysmgr, SYSMGR_IOC_EdesEn_crypt, &rdmdata) < 0) {
		fprintf(stderr,"SYSMGR_IOC_EdesEn_crypt ioctl failed:%d\n", errno);
	}
/////yan /////////
