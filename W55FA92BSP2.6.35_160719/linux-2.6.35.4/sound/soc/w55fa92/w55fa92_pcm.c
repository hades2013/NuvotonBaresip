...
static const struct snd_pcm_hardware w55fa92_pcm_hardware = {
...
static int w55fa92_dma_hw_params(struct snd_pcm_substream *substream,
                                struct snd_pcm_hw_params *params)
{
        struct snd_pcm_runtime *runtime = substream->runtime;
        struct w55fa92_audio *w55fa92_audio = runtime->private_data;
        unsigned long flags;
        int ret = 0;
	...
	ENTER();
        ...
        if(runtime->dma_addr == 0) {
        	...
        	w55fa92_audio->substream[substream->stream] = substream;        	
        }

        w55fa92_audio->dma_addr[substream->stream] = runtime->dma_addr;
        w55fa92_audio->buffersize[substream->stream] =
                params_buffer_bytes(params);
	printk("SPU allocate buffer size: %d\n",params_buffer_bytes(params));
        spin_unlock_irqrestore(&w55fa92_audio->irqlock, flags);

        return ret;
}

static void w55fa92_update_dma_register(struct snd_pcm_substream *substream,
                                       dma_addr_t dma_addr, size_t count)
{

	
#ifdef CONFIG_SND_SOC_W55FA92_I2S
...
static int w55fa92_dma_hw_free(struct snd_pcm_substream *substream)
{
        struct snd_pcm_runtime *runtime = substream->runtime;
        struct w55fa92_audio *w55fa92_audio = runtime->private_data;
	...
	ENTER();
        ...
        w55fa92_audio->substream[substream->stream] = NULL;

	LEAVE();
        return 0;
}

static int w55fa92_dma_prepare(struct snd_pcm_substream *substream)
{
        struct snd_pcm_runtime *runtime = substream->runtime;
        struct w55fa92_audio *w55fa92_audio = runtime->private_data;
        unsigned long flags;
	...
	ENTER();
        ...
        w55fa92_update_dma_register(substream,
                                   w55fa92_audio->dma_addr[substream->stream],
                                   w55fa92_audio->buffersize[substream->stream]);
...
int w55fa92_dma_getposition(struct snd_pcm_substream *substream,
...
static snd_pcm_uframes_t w55fa92_dma_pointer(struct snd_pcm_substream *substream)
{
        struct snd_pcm_runtime *runtime = substream->runtime;
        dma_addr_t src, dst;
        unsigned long res;
...
//		ENTER();
		...
		spin_lock(&w55fa92_audio->lock);
        w55fa92_dma_getposition(substream, &src, &dst);
        ...
        spin_unlock(&w55fa92_audio->lock);

//		LEAVE();        
		return frames;
}

static int w55fa92_dma_open(struct snd_pcm_substream *substream)
{
        struct snd_pcm_runtime *runtime = substream->runtime;
        ...
        snd_soc_set_runtime_hwparams(substream, &w55fa92_pcm_hardware);

#ifdef CONFIG_SND_SOC_W55FA92_SPU
        w55fa92_audio = w55fa92_spu_data;
#endif

#ifdef CONFIG_SND_SOC_W55FA92_I2S
        w55fa92_audio = w55fa92_i2s_data;
#endif
        runtime->private_data = w55fa92_audio;              
		
		LEAVE();		
        return 0;
}

static int w55fa92_dma_close(struct snd_pcm_substream *substream)
{
		ENTER();
...
int w55fa92_dma_create(struct w55fa92_audio *w55fa92_audio)
{
		int ret;
	ENTER();

		DBG("w55fa92_audio->irq_num = 0x%x !!! \n", w55fa92_audio->irq_num);
...
EXPORT_SYMBOL_GPL(w55fa92_dma_create);

int w55fa92_dma_destroy(struct w55fa92_audio *w55fa92_audio)
{
	ENTER();
	
		 free_irq(w55fa92_audio->irq_num, w55fa92_audio);
		 
		 return 0;    
}
EXPORT_SYMBOL_GPL(w55fa92_dma_destroy);

static int w55fa92_dma_mmap(struct snd_pcm_substream *substream,
...
static struct snd_pcm_ops w55fa92_dma_ops = {
        .open		= w55fa92_dma_open,
        .close		= w55fa92_dma_close,
        .ioctl		= snd_pcm_lib_ioctl,
        .hw_params	= w55fa92_dma_hw_params,
        .hw_free	= w55fa92_dma_hw_free,
        .prepare	= w55fa92_dma_prepare,
        .pointer	= w55fa92_dma_pointer,
        .mmap		= w55fa92_dma_mmap,
};

static void w55fa92_dma_free_dma_buffers(struct snd_pcm *pcm)
{
	ENTER();
...
static u64 w55fa92_pcm_dmamask = DMA_BIT_MASK(32);
static int w55fa92_dma_new(struct snd_card *card,
                          struct snd_soc_dai *dai, struct snd_pcm *pcm)
{
	ENTER();
		
        if (!card->dev->dma_mask)
                card->dev->dma_mask = &w55fa92_pcm_dmamask;
        if (!card->dev->coherent_dma_mask)
                card->dev->coherent_dma_mask = DMA_BIT_MASK(32);

#ifdef CONFIG_SND_SOC_W55FA92_I2S
        snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV,
                                              card->dev, 32 * 1024, (32 * 1024) );
#else
        snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV,
...
struct snd_soc_platform w55fa92_soc_platform = {
        .name		= "w55fa92-dma",
        .pcm_ops	= &w55fa92_dma_ops,
        .pcm_new	= w55fa92_dma_new,
        .pcm_free	= w55fa92_dma_free_dma_buffers,
};
EXPORT_SYMBOL_GPL(w55fa92_soc_platform);

static int __init w55fa92_soc_platform_init(void)
{
        return snd_soc_register_platform(&w55fa92_soc_platform);
}

static void __exit w55fa92_soc_platform_exit(void)
{
        snd_soc_unregister_platform(&w55fa92_soc_platform);
}

module_init(w55fa92_soc_platform_init);
module_exit(w55fa92_soc_platform_exit);
...
#define GPIOE	4
	...
	void headset_detection(void)
	{
		...
		#ifdef CONFIG_SOUND_W55FA92_PLAY_SPU				                
	...
	int enable_earphone_detect()
	{
	#if defined (CONFIG_HEADSET_GPB2_AND_SPEAKER_GPB3)
        outl(inl(REG_GPBFUN0) & ~MF_GPB2, REG_GPBFUN0);				// headset detect plug IN/OUT
        outl(inl(REG_GPBFUN0) & ~MF_GPB3, REG_GPBFUN0);				// speaker control signal
...
//        printk("register the headset_detect_irq\n");
        ...
        outl(inl(REG_GPDFUN1) & ~MF_GPD14, REG_GPDFUN1);				// headset detect plug IN/OUT
        outl(inl(REG_GPIOD_OMD) & ~(0x4000), REG_GPIOD_OMD); 		// port D14 input
        outl(inl(REG_GPIOD_PUEN) | (0x4000), REG_GPIOD_PUEN); 		// port D14 pull-up

        outl(inl(REG_GPAFUN0) & ~MF_GPA2, REG_GPAFUN0);				// speaker control by GPA2
...
//        printk("register the headset_detect_irq\n");
        ...
        outl(inl(REG_GPEFUN0) & ~MF_GPE0, REG_GPEFUN0);				// headset detect plug IN/OUT
        outl(inl(REG_GPEFUN0) & ~MF_GPE1, REG_GPEFUN0);				// speaker control signal
...
//        printk("register the headset_detect_irq\n");
        ...
        outl(inl(REG_GPDFUN0) & ~MF_GPD3, REG_GPDFUN0);				// headset detect plug IN/OUT
        outl(inl(REG_GPIOD_OMD) & ~(0x0008), REG_GPIOD_OMD); 		// port D3 input
        outl(inl(REG_GPIOD_PUEN) | (0x0008), REG_GPIOD_PUEN); 		// port D3 pull-up

        outl(inl(REG_GPDFUN0) & ~MF_GPD4, REG_GPDFUN0);				// speaker control by GPD4
...
MODULE_DESCRIPTION("w55fa92 Audio DMA module");
MODULE_LICENSE("GPL");
