...
int w55fa92_set_apll_clock(unsigned int clock)
{
	...
	if (w55fa92_external_clock == 12000000) {
		if (clock == 430080) {
			...
			__raw_writel(0x09EB, REG_APLLCON);
		}
			...
			__raw_writel(0x0B7F, REG_APLLCON);
		}
		else if (clock == 225792) {
			// for SPU/I2S 44.1/88.2/176.4KHz * 256, TD = 0.0850%
			__raw_writel(0x0ADE, REG_APLLCON);
		}
		else if (clock == 208896) {
			// for I2S, TD = 0.0498%
			__raw_writel(0x11E8, REG_APLLCON);
		}
		else if (clock == 169344) {
			// for I2S, TD = 0.0921%
			__raw_writel(0x1271, REG_APLLCON);
		}
		else if (clock == 147456) {
			// for I2S, TD = 0.0977%
			__raw_writel(0x12FB, REG_APLLCON);
		}
