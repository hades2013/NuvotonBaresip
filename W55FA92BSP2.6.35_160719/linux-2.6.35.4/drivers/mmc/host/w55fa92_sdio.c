...
#ifdef NVT_SD_SD0
    if (host->port == 0)
        host->present = nvt_sd_read(REG_SDIOISR) & SDISR_CD_Card;
#endif
...
static void nvt_sdio_enable(struct nvt_sd_host *host)
{
  ...
  #elif defined (CONFIG_SDIO1_CD_GPG5_SW)
    // SDIO 1 GPIO select: set GPG2~3, 12~15 to SDIO card mode (SD1_CLK/SD1_CMD/DAT1[0~3]).
    // Set GPG4 to GPIO mode to disable SDIO port 1 card detection feature.
    nvt_sd_write(REG_GPGFUN0, (nvt_sd_read(REG_GPGFUN0) & (~0x000FFF00)) | 0x00001100);
    nvt_sd_write(REG_GPGFUN1, (nvt_sd_read(REG_GPGFUN1) & (~0xFFFF0000)) | 0x11110000);
...
static int sdio_event_thread(void *unused)
{
    ...
    //  1. Initail SD driver (nvt_sdio_init()) and don't handle SD card insert event;
    ...
    //  4. Detect SD card status in thread sdio_event_thread() to trigger SD card event
            ...
            if (int_status & SDISR_CRC_IF)
            {
                ...
                if ((host->cmd->opcode != 51) && (host->cmd->opcode != 6))
                {
                    ...
                    if ((nvt_sd_read(REG_SDIOISR) & SDISR_CRC_16) == 0)
                        nvt_sd_debug("CRC-16 error detected and set to %d (cmd = %d, retries = %d, SDIOISR=0x%08X)\n",
