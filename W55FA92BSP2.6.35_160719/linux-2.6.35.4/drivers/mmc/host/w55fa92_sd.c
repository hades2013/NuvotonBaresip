/*
 *  linux/drivers/mmc/host/w55fa92_sd.c - Nuvoton W55FA92 SD Driver
 *
...
#define DATE_CODE   "20140811"

//--- Define compile flags depend on Kconfig
#ifdef CONFIG_FA92_SD_SD0_SD1
    #define NVT_SD_SD1
#elif defined (CONFIG_FA92_SD_SD0_SD2)
    #define NVT_SD_SD2
#endif
...
#define DRIVER_NAME "w55fa92-fmi"   // driver name MUST exact same to device name that defined at dev.c
...
static void nvt_sd_set_clock(u32 sd_clock_khz);
static void nvt_sd_change_driver_strength(int card_no, u32 sd_clock_khz);
...
static int nvt_sd_card_detect(struct mmc_host *mmc)
{
  ...
  #elif defined (CONFIG_SD0_CD_GPD6)
        if (nvt_sd_read(REG_GPIOD_PIN) & BIT6)
            host->present = 1;  // GPD6 high means card removed.
  #elif defined (CONFIG_SD0_CD_GPE10)
        if (nvt_sd_read(REG_GPIOE_PIN) & BIT10)
            host->present = 1;  // GPE10 high means card removed.
  #endif
    }
  ...
  #elif defined (CONFIG_SD1_CD_GPD6)
        if (nvt_sd_read(REG_GPIOD_PIN) & BIT6)
            host->present = 1;  // GPD6 high means card removed.
  #endif
    }
    ...
    else if (host->port == 2)
    {
        ...
        if (nvt_sd_read(REG_GPIOA_PIN) & BIT3)
            host->present = 1;  // GPA3 high means card removed.
  #elif defined (CONFIG_SD2_CD_GPD6)
        if (nvt_sd_read(REG_GPIOD_PIN) & BIT6)
            host->present = 1;  // GPD6 high means card removed.
  #elif defined (CONFIG_SD2_CD_GPD6_SW)
        if (!(nvt_sd_read(REG_GPIOD_PIN) & BIT6))
            host->present = 1;  // GPD6 low means card removed.
  #endif
    }
#endif
...
static void nvt_sd_enable(struct nvt_sd_host *host)
{
    ...
    nvt_sd_write(REG_GPEFUN0, (nvt_sd_read(REG_GPEFUN0) & (~0xFFFFFF00)) | 0x22222200); // SD0_CLK/CMD/DAT0_3 pins selected

#ifdef CONFIG_SD0_CD_GPA1
    // set GPA1 to SD mode for SD port 0 card detection
    nvt_sd_write(REG_GPAFUN0, (nvt_sd_read(REG_GPAFUN0) & (~MF_GPA1)) | 0x00000020);    // set GPA1 to 0010b for SD0 card detection
    nvt_sd_write(REG_SDIER, nvt_sd_read(REG_SDIER) | SDIER_CDSRC);      // set GPIO as SD card detect pin
#elif defined (CONFIG_SD0_CD_GPD6)
    //--- Select GPD6 as SD port 0 card detection. HIGH means card removed, LOW means inserted.
    // Set GPD6 to GPIO mode for SD port 0 card detection.
    nvt_sd_write(REG_GPDFUN0, nvt_sd_read(REG_GPDFUN0) & (~MF_GPD6));  // set GPD6 to GPIO mode
    nvt_sd_write(REG_GPIOD_PUEN, nvt_sd_read(REG_GPIOD_PUEN) | BIT6);  // set GPD6 internal pull high
    nvt_sd_write(REG_GPIOD_OMD, nvt_sd_read(REG_GPIOD_OMD) & (~BIT6)); // set GPD6 to input mode

    // Set GPD6 as interrupt pin and bind to GPIO1 interrupt
    nvt_sd_write(REG_IRQTGSRC1, BIT22);     // clear GPD6 interrupt status
    nvt_sd_write(REG_IRQSRCGPD, (nvt_sd_read(REG_IRQSRCGPD) & (~0x00003000)) | 0x00001000);   // set GPD6 as GPIO1 interrupt trigger source
    nvt_sd_write(REG_IRQENGPD, nvt_sd_read(REG_IRQENGPD) | BIT6 | BIT22);  // set GPD6 trigger by both falling and rising edge
#elif defined (CONFIG_SD0_CD_GPE10)
    //--- Select GPE10 as SD port 0 card detection. HIGH means card removed, LOW means inserted.
    // Set GPE10 to GPIO mode for SD port 0 card detection.
    nvt_sd_write(REG_GPEFUN1, nvt_sd_read(REG_GPEFUN1) & (~MF_GPE10));  // set GPE10 to GPIO mode
    nvt_sd_write(REG_GPIOE_PUEN, nvt_sd_read(REG_GPIOE_PUEN) | BIT10);  // set GPE10 internal pull high
    nvt_sd_write(REG_GPIOE_OMD, nvt_sd_read(REG_GPIOE_OMD) & (~BIT10)); // set GPE10 to input mode

    // Set GPE10 as interrupt pin and bind to GPIO1 interrupt
    nvt_sd_write(REG_IRQTGSRC2, BIT10);     // clear GPE10 interrupt status
    nvt_sd_write(REG_IRQSRCGPE, (nvt_sd_read(REG_IRQSRCGPE) & (~0x00300000)) | 0x00100000); // set GPE10 as GPIO1 interrupt trigger source
    nvt_sd_write(REG_IRQENGPE, nvt_sd_read(REG_IRQENGPE) | BIT10 | BIT26);  // set GPE10 trigger by both falling and rising edge
#endif

#ifdef NVT_SD_SD1
    // SD 1 GPIO select: set GPB0~5 to SD card mode (SD1_CLK/SD1_CMD/DAT1[0~3])
    nvt_sd_write(REG_GPBFUN0, (nvt_sd_read(REG_GPBFUN0) & (~0x00FFFFFF)) | 0x00222222);
#endif

#ifdef NVT_SD_SD2
    //SD 2 GPIO select: set GPD5,7,8 and GPE10~12 to SD card mode (SD2_CLK/SD2_CMD/DAT2[0~3])
    nvt_sd_write(REG_GPDFUN0, (nvt_sd_read(REG_GPDFUN0) & (~0xF0F00000)) | 0x10100000);  // SD2_DAT2/SD2_CLK pins selected
    nvt_sd_write(REG_GPDFUN1, (nvt_sd_read(REG_GPDFUN1) & (~0x0000000F)) | 0x00000001);  // SD2_CMD pins selected
    nvt_sd_write(REG_GPEFUN1, (nvt_sd_read(REG_GPEFUN1) & (~0x000FFF00)) | 0x00011100);  // SD2_DAT0_1_3 pins selected
#endif
    ...
    nvt_sd_write(REG_GPAFUN0, nvt_sd_read(REG_GPAFUN0) & (~MF_GPA3));   // set GPA3 to GPIO mode
  ...
  #elif defined (CONFIG_SD1_CD_GPD6) || defined (CONFIG_SD2_CD_GPD6)
    //--- Select GPD6 as SD port 1/2 card detection. HIGH means card removed, LOW means inserted.
    // Set GPD6 to GPIO mode for SD port 1/2 card detection.
    nvt_sd_write(REG_GPDFUN0, nvt_sd_read(REG_GPDFUN0) & (~MF_GPD6));  // set GPD6 to GPIO mode
    nvt_sd_write(REG_GPIOD_PUEN, nvt_sd_read(REG_GPIOD_PUEN) | BIT6);  // set GPD6 internal pull high
    nvt_sd_write(REG_GPIOD_OMD, nvt_sd_read(REG_GPIOD_OMD) & (~BIT6)); // set GPD6 to input mode
    ...
    nvt_sd_write(REG_IRQENGPD, nvt_sd_read(REG_IRQENGPD) | BIT6 | BIT22);  // set GPD6 trigger by both falling and rising edge
  #elif defined (CONFIG_SD2_CD_GPD6_SW)
    //--- Select GPD6 OUTPUT mode as SD port 2 card detection. LOW means card removed, HIGH means inserted.
    // Set GPD6 to GPIO mode for SD port 2 card detection.
    nvt_sd_write(REG_GPDFUN0, nvt_sd_read(REG_GPDFUN0) & (~MF_GPD6));   // set GPD6 to GPIO mode
    nvt_sd_write(REG_GPIOD_PUEN, nvt_sd_read(REG_GPIOD_PUEN) | BIT6);   // set GPD6 internal pull high
    nvt_sd_write(REG_GPIOD_DOUT, nvt_sd_read(REG_GPIOD_DOUT) | BIT6);   // set GPD6 to output high
    nvt_sd_write(REG_GPIOD_OMD, nvt_sd_read(REG_GPIOD_OMD) | BIT6);     // set GPD6 to output mode
...
#ifdef CONFIG_FA92_SD0_WP
    //--- Initail GPA0 for SD card 0 write protect
    if (host->port == 0)
    {
        // set GPA0 to GPIO mode for SD port 0 write protect
        nvt_sd_write(REG_GPAFUN0, nvt_sd_read(REG_GPAFUN0) & (~MF_GPA0));   // set GPA0 to GPIO mode for write protect
        nvt_sd_write(REG_GPIOA_OMD, nvt_sd_read(REG_GPIOA_OMD) & (~BIT0));  // set GPA0 to input mode
    }
#elif defined (CONFIG_FA92_SD0_WP_GPE11)
    //--- Initail GPE11 for SD card 0 write protect
    if (host->port == 0)
    {
        // set GPE11 to GPIO mode for SD port 0 write protect
        nvt_sd_write(REG_GPEFUN1, nvt_sd_read(REG_GPEFUN1) & (~MF_GPE11));   // set GPE11 to GPIO mode for write protect
...
static void nvt_sd_send_command(struct nvt_sd_host *host, struct mmc_command *cmd)
{
     ...
     * the data sheet says
     */
    nvt_sd_change_driver_strength(host->port, host->mmc->ios.clock / 1000);
...
/*-----------------------------------------------------------------------------
 * 2013/9/6, change SD pins driver strength according to the SD clock.
...
static void nvt_sd_change_driver_strength(int card_no, u32 sd_clock_khz)
{
    if (sd_clock_khz <= 25000)
    {
        ...
        if (card_no == 0)       // GPE2~7 for SD0 pins
            nvt_sd_write(REG_MISC_DS_GPE, nvt_sd_read(REG_MISC_DS_GPE) & ~0x000000FC);
        else if (card_no == 1)  // GPB0~5 for SD1 pins
            nvt_sd_write(REG_MISC_DS_GPB, nvt_sd_read(REG_MISC_DS_GPB) & ~0x0000003F);
        else if (card_no == 2)  // GPD5,7,8 and GPE10~12 for SD2 pins
        {
            nvt_sd_write(REG_MISC_DS_GPD, nvt_sd_read(REG_MISC_DS_GPD) & ~0x000001A0);
            nvt_sd_write(REG_MISC_DS_GPE, nvt_sd_read(REG_MISC_DS_GPE) & ~0x00001C00);
        }
    }
    else
    {
...
static void nvt_sd_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    switch (ios->power_mode) {
        ...
        case MMC_POWER_ON:
            ...
            nvt_sd_write(REG_FMICR, FMI_SD_EN);
            nvt_sd_change_driver_strength(host->port, ios->clock / 1000);
            if (ios->clock == 0)
                nvt_sd_set_clock(300);      // default SD clock 300KHz
            else
...
static int sd_event_thread(void *unused)
{
    ...
    //  1. Initail SD driver (nvt_sd_init()) and don't handle SD card insert event;
    //  2. /etc/init.d/rcS will create file /dev/mdev.seq when system ready to mount disk;
    //  3. Don't report and handle any SD event before step 1 and 2.
    //  4. Detect SD card status in thread sd_event_thread() to trigger SD card event
    //     after file /dev/mdev.seq exist.
    //-----------------------------------------------------------------------------------
    //--- Waiting for nvt_sd_init() completed
    while (sd_init_completed == 0)
        schedule();

    //--- Waiting for /etc/init.d/rcS to create file /dev/mdev.seq
    //--- File /dev/mdev.seq exist means system ready to mount SD card.
    while(1)
    {
        fp = filp_open("/dev/mdev.seq", O_RDONLY, 0);
        if (IS_ERR(fp))
        {
            schedule(); // wait if file /dev/mdev.seq not exist
        }
        else
        {
            filp_close(fp, NULL);
            mmc_detect_change(sd_host->mmc, msecs_to_jiffies(500));     // detect SD card status
            break;  // go ahead if file /dev/mdev.seq exist
        }
    }
...
#ifdef CONFIG_SD0_CD_GPE10
    src = nvt_sd_read(REG_IRQTGSRC2);
    if (src & BIT10)    // This interrupt is trigger by GPE10
    {
        // nvt_sd_debug("--> nvt_sd0_card_detect_irq() GPE10 interrupt happened !\n");
        if (nvt_sd_read(REG_GPIOE_PIN) & BIT10)
            host->present = 1;  // card removed
        else
...
static irqreturn_t nvt_sd1_card_detect_irq(int irq, void *devid)
{
    ...
    if (src & BIT22) // This interrupt is trigger by GPD6
    {
        // nvt_sd_debug("--> nvt_sd1_card_detect_irq() GPD6 interrupt happened !\n");
        if (nvt_sd_read(REG_GPIOD_PIN) & BIT6)
            host->present = 1;  // card removed
        else
            host->present = 0;  // card inserted
...
static irqreturn_t nvt_sd2_card_detect_irq(int irq, void *devid)
{
    ...
    if (src & BIT22) // This interrupt is trigger by GPD6
    {
        // nvt_sd_debug("--> nvt_sd2_card_detect_irq() GPD6 interrupt happened !\n");
        if (nvt_sd_read(REG_GPIOD_PIN) & BIT6)
            host->present = 1;  // card removed
        else
            host->present = 0;  // card inserted
    ...
    if (src & BIT22)    // This interrupt is trigger by GPD6
    {
        //nvt_sd_debug("--> nvt_sd2_card_detect_irq() GPD6 OUTPUT mode interrupt happened !\n");
        if (nvt_sd_read(REG_GPIOD_DOUT) & BIT6)
            host->present = 0;  // card inserted
        else
            host->present = 1;  // card removed
        /* 0.5s needed because of early card detect switch firing */
...
static int nvt_sd_get_ro(struct mmc_host *mmc)
{
    ...
    // use GPE11 as the write protect pin for SD port 0
    if (host->port == 0)
    {
        if ((nvt_sd_read(REG_GPIOE_PIN) & BIT11) == 0)
        {
            // nvt_sd_debug("nvt_sd_get_ro(): SD port %d is write unprotected (GPE11).\n", host->port);
            return 0;   // write unprotected
        }
        else
        {
            // nvt_sd_debug("nvt_sd_get_ro(): SD port %d is write protected (GPE11).\n", host->port);
            return 1;   // write protected
        }
    }
#endif
...
static int __init nvt_sd_probe(struct platform_device *pdev)
{
  ...
  #if defined (CONFIG_SD0_CD_GPE10) || defined (CONFIG_SD0_CD_GPD6)
  ...
  #if defined (CONFIG_SD2_CD_GPA3) || defined (CONFIG_SD2_CD_GPD6) || defined (CONFIG_SD2_CD_GPD6_SW)
    ...
    nvt_sd_debug("W55FA92 MMC/SD driver (%s) has been initialized successfully!\n", DATE_CODE);
...
#else
    // Disable Card Detect interrupt of SD engine since we don't use GAP1 as card detect pin now.
    nvt_sd_write(REG_SDIER, nvt_sd_read(REG_SDIER) & (~SDIER_CD_IEN));  // disable Interrupt for card detect
#endif
...
MODULE_DESCRIPTION("W55FA92 SD Card Interface driver");
MODULE_AUTHOR("HPChen / CJChen");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:w55fa92_sd");
