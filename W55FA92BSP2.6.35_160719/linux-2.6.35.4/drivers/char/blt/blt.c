...
static PFN_BLT_CALLBACK _bltCmpltCb			=	NULL;
static PFN_BLT_CALLBACK _bltMMUPgFltCb		=	NULL;
static PFN_BLT_CALLBACK _bltMMUPgMsCb		=	NULL;

static unsigned long _bltCmpltCbUsrDat		= 0;
static unsigned long _bltMMUPgFltCbUsrDat	= 0;
static unsigned long _bltMMUPgMsCbUsrDat	= 0;

static struct clk *clk_hclk4 = NULL;
static struct clk *clk_blt = NULL;
...
void _blt_intr_hdlr (void)
{
	...
	if (bltIntStat & BLT_PFT_INTS) {	// BLT MMU page fault
		...
		if (_bltMMUPgFltCb) {
			_bltMMUPgFltCb (_bltMMUPgFltCbUsrDat);
		}
	}
	...
	if (bltIntStat & BLT_PMS_INTS) {	// BLT MMU page miss
		...
		if (_bltMMUPgMsCb) {
			_bltMMUPgMsCb (_bltMMUPgMsCbUsrDat);
		}
	}
}
...
ERRCODE bltOpen(void)
{
	...
	BUG_ON(IS_ERR(clk_hclk4));
	clk_enable(clk_hclk4);
	// Enable BLT clock.
	clk_blt = clk_get(NULL, "blt");
	BUG_ON(IS_ERR(clk_blt));
	clk_enable(clk_blt);
...
void bltInstallCallback(E_BLT_INT_TYPE eIntType, PFN_BLT_CALLBACK newCb, unsigned long newCbUsrDat, PFN_BLT_CALLBACK *oldCb_p, unsigned long *oldCbUsrDat_p)
{
	if (eIntType == BLT_INT_CMPLT) {	// BLT complete
		...
		if (oldCbUsrDat_p) {
			*oldCbUsrDat_p = _bltCmpltCbUsrDat;
		}
		_bltCmpltCbUsrDat = newCbUsrDat;
	}	
	else if(eIntType == BLT_INT_PGFLT) {	// BLT MMU page fault
		if (oldCb_p) {
			*oldCb_p = _bltMMUPgFltCb;
		}
		_bltMMUPgFltCb = newCb;
		
		if (oldCbUsrDat_p) {
			*oldCbUsrDat_p = _bltMMUPgFltCbUsrDat;
		}
		_bltMMUPgFltCbUsrDat = newCbUsrDat;
	}
	else if (eIntType == BLT_INT_PGMS) {	// BLT page miss
		if (oldCb_p) {
			*oldCb_p = _bltMMUPgMsCb;
		}
		_bltMMUPgMsCb = newCb;
		
		if (oldCbUsrDat_p) {
			*oldCbUsrDat_p = _bltMMUPgMsCbUsrDat;
		}
		_bltMMUPgMsCbUsrDat = newCbUsrDat;
	}
}
...
{
	srcimg_p->u32SrcImageAddr = inp32(REG_SADDR);		// Get Source Image Start Addr
	srcimg_p->i32Stride = inp32(REG_SSTRIDE);			// Get Source Image Stride	
	srcimg_p->i32XOffset = inp32(REG_OFFSETSX);			// Get X offset into the source to start rendering from
	srcimg_p->i32YOffset = inp32(REG_OFFSETSY);			// Get Y offset into the source to start rendering from
	srcimg_p->i16Width = inp32(REG_SWIDTH);				// Get width of source image
	srcimg_p->i16Height = inp32(REG_SHEIGHT);			// Get height of source image
}
...
{
	frmbuff_p->u32FrameBufAddr = inp32(REG_DADDR);		// Get Frame Buffer Start Addr
	frmbuff_p->i32Stride = inp32(REG_DSTRIDE);			// Get Frame Buffer Stride
	frmbuff_p->i16Width = inp32(REG_DWIDTH);			// Get width of Frame Buffer
	frmbuff_p->i16Height = inp32(REG_DHEIGHT);			// Get height of Frame Buffer
}
...
{
	return (inp32(REG_SET2DA) & L_ENDIAN) >> 1;
}
...
)
{
	...
	u32PaletteStartInx = (UINT32) REG_PALETTE + u32PaletteInx;
	for(i=0;i<u32Num;i++)
	{
...
)
{
	...
	u32PaletteStartInx = (UINT32) REG_PALETTE + u32PaletteInx;
	for (i = 0; i < u32Num; i ++) {
		u32ARGB = inp32(u32PaletteStartInx + i*4);
		psARGB->u8Alpha = u32ARGB >> 24;
		psARGB->u8Red = (u32ARGB >> 16) & 0xFF;
		psARGB->u8Green = (u32ARGB >> 8) & 0xFF;
		psARGB->u8Blue = u32ARGB & 0xFF;
...
{
	return (inp32(REG_SET2DA) & FILL_STYLE) >> 8;
}
...
static void _pgflt_hdlr (unsigned long usrDat)
{
	...
	{	// Fill in the selected TTB entry.
		...
		bltmmuSetTTBEntry (ttbIdx, ((UINT32 *) __va(ttb))[lv1PgTblIdx]);
	}
...
static void _pgms_hdlr (unsigned long usrDat)
{
	_pgflt_hdlr (usrDat);
}
...
static unsigned long _old_pgflt_hdlr_usrdat = 0;
static unsigned long _old_pgms_hdlr_usrdat = 0;
...
void bltmmuSetup (UINT32 ttb, BOOL mainTLB, BOOL mainTLBSrcCh)
{
	...
	bltInstallCallback (BLT_INT_PGFLT, _pgflt_hdlr, 0, &_old_pgflt_hdlr, &_old_pgflt_hdlr_usrdat);						
	bltInstallCallback (BLT_INT_PGMS, _pgms_hdlr, 0, &_old_pgms_hdlr, &_old_pgms_hdlr_usrdat);
...
void bltmmuTeardown (void)
{
	...
	bltInstallCallback (BLT_INT_PGFLT, _old_pgflt_hdlr, _old_pgflt_hdlr_usrdat, &_old_pgflt_hdlr, &_old_pgflt_hdlr_usrdat);						
	bltInstallCallback (BLT_INT_PGMS, _old_pgms_hdlr, _old_pgms_hdlr_usrdat, &_old_pgms_hdlr, &_old_pgms_hdlr_usrdat);
}
