...
int w55fa92_aac_open(struct inode *inode, struct file *filp)
{
	...
	do {
		...
		priv->buff_relay_va = dma_alloc_coherent(NULL, MAX_AAC_BUFF * 2, (dma_addr_t *) &priv->buff_relay_pa, GFP_KERNEL);
		if (! priv->buff_relay_va) {
			printk(KERN_ERR "[AAC Driver] dma_alloc_coherent 0x%08x failed.\n", MAX_AAC_BUFF * 2);
			err = -ENOMEM;
			break;
		}
...
int w55fa92_aac_close(struct inode *inode, struct file *filp)
{
	...
	if (priv) {
		if (priv->buff_relay_va) {
			dma_free_coherent(NULL, MAX_AAC_BUFF * 2, priv->buff_relay_va, priv->buff_relay_pa);
...
int w55fa92_aac_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case AAC_IOCSENC:
		...
		if ((priv->encoder.i32Size << 2 ) > MAX_AAC_BUFF) {
			printk(KERN_ERR "[AAC Driver] %d larger than max encoder buffer size: %d.\n", priv->encoder.i32Size, MAX_AAC_BUFF);
			err = -EFAULT;
			break;
		}
		...
		flush_cache_all();
        DrvAAC_Encoder((INT32 *) priv->buff_relay_pa, (INT32 *) (priv->buff_relay_pa + MAX_AAC_BUFF), priv->encoder.i32Size);
		if (copy_to_user(priv->encoder.outbuf, (void *) (((unsigned long) priv->buff_relay_va) + MAX_AAC_BUFF), priv->encoder.i32Size << 1)) {
			err = -EFAULT;
			break;
		}
	...
	case AAC_IOCSDEC:
		...
		flush_cache_all();
        DrvAAC_Decoder(priv->decoder.i32Size, (INT32 *) priv->buff_relay_pa, (INT32 *) (priv->buff_relay_pa + MAX_AAC_BUFF));
		if (copy_to_user(priv->decoder.outbuf, (void *) (((unsigned long) priv->buff_relay_va) + MAX_AAC_BUFF), priv->decoder.i32Size << 2)) {
			err = -EFAULT;
			break;
		}
