/* linux/driver/input/w55fa92_sysmgr.c
 *
...
#define SYSMGR_DRIVER_NAME	"w55fa92-sysmgr"
...
char w55fa92_nand_dev = '-';
char w55fa92_sd_dev = '-';
EXPORT_SYMBOL(w55fa92_nand_dev);
EXPORT_SYMBOL(w55fa92_sd_dev);

 unsigned long  libbackdata;
 unsigned long rdmdata=0x33cc55aa;//随机数，填时间，声音，电压等随机数
...
int w55fa92_sysmgr_open(struct inode* i,struct file* file)
{
...
int w55fa92_sysmgr_close(struct inode* i,struct file* file)
{
	int num;
...
ssize_t w55fa92_sysmgr_read(struct file *file, char *buff, size_t read_mode, loff_t *offp)
{
...
ssize_t w55fa92_sysmgr_write(struct file *file, const char *buf, size_t count, loff_t *f_pos)
{
	unsigned int cmd;
...
static int w55fa92_sysmgr_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
...
////////////////////////////////////////
	case SYSMGR_IOC_EdesEn_get_rdmdata:
		if (copy_to_user((void *)arg, (void *)&rdmdata, sizeof(unsigned int))) {
			ret = -EFAULT;
			break;
		}
		break;

	case SYSMGR_IOC_EdesEn_crypt:
			if (copy_from_user((void *)&cmd_data, (void *)arg, sizeof(unsigned int))) {
				ret = -EFAULT;
				break;
			}
			printk("edes1:%x",cmd_data);
			if (cmd_data == libbackdata)
						printk("check ok\n");
			else
				{
					printk("check err\n");
...
///////////////////////////////////////
	...
	case SYSMGR_IOC_GET_NAND_DEV:
		//printk("NAND is %c\n", w55fa92_nand_dev);
		if (copy_to_user((char *)arg, (void *)&w55fa92_nand_dev, sizeof(char))) {
			ret = -EFAULT;
			break;
		}
		break;
	...
	case SYSMGR_IOC_GET_SD_DEV:
		//printk("SD is %c\n", w55fa92_sd_dev);
		if (copy_to_user((char *)arg, (void *)&w55fa92_sd_dev, sizeof(char))) {
			ret = -EFAULT;
			break;
		}
		break;
	...
	case SYSMGR_IOC_GET_POWER_KEY:
		...
		while(1) {	
			...
			if ((readl(REG_RTC_AER) & 0x10000) == 0x0) {
				...
				// FA92 does not have REG_RTC_REG_FLAG	
				//rtc_wait_ready();
...
static unsigned int w55fa92_sysmgr_poll(struct file *file, poll_table *wait)
{
	unsigned int mask = 0;
...
struct file_operations w55fa92_sysmgr_fops = {
owner:
	THIS_MODULE,
open:
	w55fa92_sysmgr_open,
read:
	w55fa92_sysmgr_read,
write:
	w55fa92_sysmgr_write,
ioctl:
	w55fa92_sysmgr_ioctl,
poll:
	w55fa92_sysmgr_poll,
release:
	w55fa92_sysmgr_close,
};
...
static int __init w55fa92_sysmgr_reg(void)
{
	int err, i;
	...
	do {
		...
		cdev_init(&sysmgr_dev, &w55fa92_sysmgr_fops);
	...
	printk("w55fa92 SysMgr driver has been initialized successfully!\n");

	return err;
}

static void __exit w55fa92_sysmgr_exit(void)
{
	sysmgr_cleanup();
}

module_init(w55fa92_sysmgr_reg);
module_exit(w55fa92_sysmgr_exit);

MODULE_DESCRIPTION("W55FA92 SysMgr driver");
MODULE_LICENSE("GPL");
