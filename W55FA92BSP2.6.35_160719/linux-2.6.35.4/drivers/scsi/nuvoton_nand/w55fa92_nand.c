...
/* linux/driver/scsi/nuvoton_nand/w55fa92_nand.c
*
...
#define DATE_CODE   "20140416"

#if 1   // for nand card
#define NAND_CD_IRQ_NUM W55FA92_IRQ(3)  // nIRQ1
#endif
...
#if defined(CONFIG_W55FA92_NAND0)
    #define ONE_NAND

#elif defined(CONFIG_W55FA92_NAND1) && defined(CONFIG_NANDCARD_SCSI)
    #define ONE_NANDCARD

#elif defined(CONFIG_W55FA92_NAND_BOTH) && defined(CONFIG_NANDCARD_SCSI)
    #define ONE_NAND
    #define TWO_NAND
    #define NANDCARD_2_DETECT

#elif defined(CONFIG_W55FA92_NAND1) && defined(CONFIG_NANDCARD_MTD)
    #define ONE_NANDCARD_MTD

#elif defined(CONFIG_W55FA92_NAND_BOTH) && defined(CONFIG_NANDCARD_MTD)
    #define ONE_NAND
    #define TWO_NAND_MTD
...
#if defined(CONFIG_NANDCARD_MTD)
    extern int w55fa92_nand_init();
    extern void w55fa92_nand_exit();
#endif
...
#if defined(ONE_NAND) || (defined(ONE_NANDCARD) && defined(CONFIG_W55FA92_CS0_RB0_PIN))
...
#if defined (TWO_NAND) || (defined(ONE_NANDCARD) && defined(CONFIG_W55FA92_CS1_RB1_PIN))
...
// Define the MAX size of buffer _fmi_pNANDBuffer.
//      _fmi_pNANDBuffer is a pointer to dynamic buffer that used for NAND page read/write.
//      So, please make sure it is equal to or larger than the MAX PAGE SIZE of NAND that you want to supported.
#define MAX_SIZE_fmi_pNANDBuffer   8192
...
char volatile _fmi_bIsSMPRegionDetect = FALSE;

#if defined(NANDCARD_2_DETECT) || defined(ONE_NANDCARD) || defined(NANDCARD_2_DETECT_MTD) || defined(ONE_NANDCARD_MTD)
// Simply return really Nand Card status in this moment that don't care about card debounce issue.
// If you care about card debounce issue, refer to nand_card_detect_irq().
...
static irqreturn_t fmi_interrupt(int irq, void *dev_id)
{
    ...
    // support Protect Region feature
    else if ((ier & SMIER_PROT_REGION_WR_IE) && (isr & SMISR_PROT_REGION_WR_IF))
    {
...
static int nand_card_init(void)
{
        ...
        MSG2("Initial NAND Card. Result = %d\n", retval);
        LEAVE();
        return retval;
}
...
#if defined (CONFIG_W55FA92_NAND_BOTH)
...
#if defined(CONFIG_W55FA92_SYSMGR) || defined (CONFIG_W55FA92_SYSMGR_MODULE)
            w55fa92_nand_dev = cmd->request->rq_disk->disk_name[2];
#endif
        }
#else
...
static int nand_scsi_write(struct scsi_cmnd  *cmd, struct nand_hostdata *dev)
{
    ...
    #ifdef CONFIG_W55FA92_NAND1_WRITE_ENABLE
        if (1) {
    #else
        if (dev->myID == 0) {
    #endif
...
static void nand_scsi_mode_sense(struct nand_hostdata *dev)
{
        ...
        if (dev->myID == 0) {
                if ( nand_flagSetting.bCardExist ) {
                        ...
                        nand_make_sense_buffer(dev, 0x02, 0x3a, 0x00 );
                        MSG( "NAND card 0 may not be inserted!\n" );
                }
        } // myID = 0
        ...
        if (dev->myID == 1) {
                if ( nand_flagSetting.bCardExist1 ) {
                        ...
                        nand_make_sense_buffer(dev, 0x02, 0x3a, 0x00 );
                        MSG( "NAND card 1 may not be inserted!\n" );
                }
        } // myID = 1
        LEAVE();
}
...
static void nand_scsi_read_capacity(struct nand_hostdata *dev)
{
        ...
        if (nand_test_unit_ready(dev)) {
                MSG( "SCSI_READ_CAPACITY - The NAND unit not ready\r\n" );
                return;
        }
        memset(buf, 0, 8);
...
static const char* nand_info( struct Scsi_Host * psh)
{
        ENTER();
        LEAVE();
        return "Nuvoton W55FA92 On Board NAND DRIVER!";
}
...
static const char* nand_info1( struct Scsi_Host * psh)
{
        ENTER();
        LEAVE();
        return "Nuvoton W55FA92 NAND Card DRIVER!";
}
...
static int sd_add_card(void)
{
        ...
        if ((sd_add == 0 )&&(nand_flagSetting.curCard == 0)) { //add card 0
                ...
                if (err)
                {
                        printk("NAND ERROR: add_card 0 FAIL !!!\n");
                        return err;
                }
        ...
        if ((sd_add1 == 0 )&&(nand_flagSetting.curCard == 1)) { //add card 1
                ...
                if (err)
                {
                        printk("NAND ERROR: add_card 1 FAIL !!!\n");
                        return err;
                }
...
#if (defined(ONE_NANDCARD) && defined(CONFIG_W55FA92_CS0_RB0_PIN))
...
#elif (defined(ONE_NANDCARD) && defined(CONFIG_W55FA92_CS0_RB0_PIN))
    static int nand_event_thread(void *unused)
    {
            int event;
            ...
            for (;;) {
                    ...
                    case SD_EVENT_ADD:
                            ...
                            if (card_exist == 1) {
                                    ...
                                    {
                                        // 2012/9/28, force to sd_del_card() if Nand card be removed during initial it.
...
static int __init nand_init(void)
{
    ...
    clk_enable(nand_clk);
        ...
        outpw(REG_GPDFUN0, (inpw(REG_GPDFUN0) & (~0xF0000000)) | 0x20000000);   // enable NRE pins
        outpw(REG_GPDFUN1, (inpw(REG_GPDFUN1) & (~0x0000000F)) | 0x00000002);   // enable NWR pins
        outpw(REG_GPEFUN1, (inpw(REG_GPEFUN1) & (~0x000FFF00)) | 0x00022200);   // enable ALE/CLE/ND3 pins

    #if defined (CONFIG_W55FA92_CS0_RB0_PIN)
        outpw(REG_GPDFUN0, (inpw(REG_GPDFUN0) & (~0x00F00000)) | 0x00200000);   // enable RB0 pins
        outpw(REG_GPEFUN1, (inpw(REG_GPEFUN1) & (~0x0000000F)) | 0x00000002);   // enable CS0 pins
    #else
        outpw(REG_GPDFUN0, (inpw(REG_GPDFUN0) & (~0x0F000000)) | 0x02000000);   // enable RB1 pins
        outpw(REG_GPEFUN1, (inpw(REG_GPEFUN1) & (~0x000000F0)) | 0x00000020);   // enable CS1 pins
    #endif
#endif
        ...
        if (request_irq(IRQ_SIC, (irq_handler_t)fmi_interrupt, IRQF_SHARED, "W55FA92_NAND", (void*)1)) {
        ...
        if (request_irq(NAND_CD_IRQ_NUM, nand_card_detect_irq, IRQF_DISABLED, "FA92_NAND_CD", NULL) != 0) {
        ...
        _fmi_pNANDBuffer = (int )dma_alloc_writecombine(NULL, MAX_SIZE_fmi_pNANDBuffer, &_fmi_ucNANDBuffer, GFP_KERNEL);
        _fmi_gptr1 = _fmi_pNANDBuffer1;

        if (_gnand_pDMABuffer == 0)
            _gnand_pDMABuffer = (u8 *)kmalloc(MAX_SIZE_fmi_pNANDBuffer, GFP_KERNEL);
        ...
        printk("W55FA92 NAND driver (%s) has been initialized successfully!\n", DATE_CODE);
        LEAVE();
        return 0;
}
...
static void __exit nand_exit(void)
{
        ...
        dma_free_writecombine(NULL, MAX_SIZE_fmi_pNANDBuffer, &_fmi_pNANDBuffer, _fmi_ucNANDBuffer);
