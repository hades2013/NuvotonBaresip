...
static irqreturn_t
//w55fa92_serial_rx_chars(int irq, void *dev_id, struct pt_regs *regs)
w55fa92_serial_rx_chars(int irq, void *dev_id)
{
	...
	{
//		ret = w55fa92_serial_tx_chars(irq, dev_id, regs);
...
//static irqreturn_t w55fa92_serial_tx_chars(int irq, void *id, struct pt_regs *regs)
...
static int w55fa92_serial_startup(struct uart_port *port)
{
	...
	if (ourport->port.irq == IRQ_UART) {
		__raw_writel((__raw_readl(REG_GPAFUN1)&~(MF_GPA10|MF_GPA11))|0x3300 , REG_GPAFUN1);
//		__raw_writel(__raw_readl(REG_APBCLK)|UART1_CKE, REG_APBCLK);
	}
	else if (ourport->port.irq == IRQ_HUART) {
		// check HUART kernel option
#ifdef CONFIG_SERIAL_W55FA92_HUART
		__raw_writel((__raw_readl(REG_GPDFUN0)&~(MF_GPD1|MF_GPD2))|0x0110, REG_GPDFUN0);
//		__raw_writel(__raw_readl(REG_APBCLK)|UART0_CKE, REG_APBCLK);
#else
					  ...
					  unsigned int baud)
{
	...
	*clk    = res.src;
	...
	master_clock = w55fa92_external_clock;
				       ...
				       struct ktermios *old)
{
	...
	wr_regl(port, W55FA92_COM_MCR, 0x00);
...
static int serial_resetport_w55fa92(struct uart_port *port,
				    struct w55fa92_uartcfg *cfg)
{
	dbg("w55fa92_serial_resetport: port=%p (%08lx), cfg=%p\n",
