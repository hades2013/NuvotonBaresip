	...
	)
{
	...
	if(bIsEnableSnrClock){		
		...
		clk = clk_get(NULL, "sen1");
		clk_enable(clk);
		//clk = clk_get(NULL, "cap1");	/* In open() enable */
		//clk_enable(clk);
	}else{
...
*/		
	...
	{//Data,SCLKO, PCLK, HSYNC, VSYNC
		...
		printk("REG_GPAFUN0 = 0x%x\n", inp32(REG_GPAFUN0));
		printk("REG_GPCFUN1 = 0x%x\n", inp32(REG_GPCFUN1));
		printk("REG_GPEFUN0 = 0x%x\n", inp32(REG_GPEFUN0));
	}
...
/* For both capture port enable */ 
void video_port2_pin_init(
	BOOL bIsEnableSnrClock,
	E_VIDEOIN_SNR_SRC eSnrSrc,	
	UINT32 u32SensorFreqKHz,						//KHz unit
	E_VIDEOIN_DEV_TYPE eDevType
...
)
{
	...
	clk = clk_get(NULL, "cap1");
	clk_enable(clk);
	outp32(REG_AHBIPRST, VIN1_RST);							/* Global reset Capture */
...
#if 0
static INT32 
videoIn_GetVerticalScaleFactor(
	E_VIDEOIN_PIPE ePipe,
...
#if 0
static INT32 
videoIn_GetHorizontalScaleFactor(
	E_VIDEOIN_PIPE bPipe,
	PUINT16 pu16Numerator, 
	PUINT16 pu16Denominator
	)
...
#if 0
static void 
videoIn_GetFrameRateScaleFactor(
	PUINT8 pu8Numerator, 
...
#if 0
static void videoIn_GetColorEffect(
...
INT32 videoIn_PreviewPipeSize(UINT16 u16height, UINT16 u16width)
{
	INT32 i32ErrCode;
...
INT32 videoIn_EncodePipeSize(UINT16 u16height, UINT16 u16width)
{
	INT32 i32ErrCode;
	UINT32 u32cropwidth,  u32cropheight;
	videoIn_GetCropWinSize(&u32cropheight, &u32cropwidth);	
...
VINDEV_T nvt_vin2 =
{
	...
	videoIn_SetPacketFrameBufferControl, 	//void (*SetPacketFrameBufferControl)(BOOL bFrameSwitch, BOOL bFrameBufferSel);
	videoIn_SetSensorPolarity,			//void (*SetSensorPolarity)(BOOL bVsync, BOOL bHsync, BOOL bPixelClk);
