...
//extern volatile videoin_motion_t videoIn_md_Offset_info[];
//extern volatile videoIn_buf_t videoIn_diff_buf[];
#endif
											...
											void *arg)
{
	...
	{
		DBG_PRINTF("Specified width must less or equal priv->i32packet_width\n");
		return 0;
	}	
	if(priv->videowin.height>priv->i32packet_height)
	{
		DBG_PRINTF("Specified height must less or equal priv->i32packet_height\n");
		return 0;
	}

	for(i=0;i<CONFIG_VIN_DEV2_PREVIEW_BUF_NO;i++)
	{/* Capture engine only supoort 2-byte/pixel */
		if(priv->i32UserPacketStride==0)
			priv->pFramePackerAddr[i] = priv->videoIn_preview_buf[i].u32PhysAddr+ priv->i32packet_width*priv->videowin.y*2 + priv->videowin.x*2; 
		else
			priv->pFramePackerAddr[i] = priv->videoIn_preview_buf[i].u32PhysAddr+ priv->i32UserPacketStride*priv->videowin.y*2 + priv->videowin.x*2;
	...
	priv->pDevVin->SetStride(priv->videowin.width, u32PlaStride);	// The stride is set by set view window cmd. 
		...
		if(priv->i32UserPacketStride == 0)//New create for dual sensor
			priv->pDevVin->SetStride(LCDWIDTH, u32PlaStride);				// preview on depends on the LCM's width 
		else
			priv->pDevVin->SetStride(priv->i32UserPacketStride, u32PlaStride);
	#else
		if(priv->i32UserPacketStride == 0){//New create for dual sensor
	...
	//Default.
#if 0 //Should be set in each sensor.c
	...
	if(priv->videopic.palette == VIDEO_PALETTE_YUV422)
		priv->pDevVin->SetDataFormatAndOrder(eVIDEOIN_IN_UYVY, eVIDEOIN_IN_YUV422, eVIDEOIN_OUT_YUV422);
#endif
#if defined(CONFIG_SENSOR_OV9660_DEV2) || defined(CONFIG_SENSOR_NT99140_DEV2)
	if(priv->videopic.palette == VIDEO_PALETTE_RGB565)
		priv->pDevVin->SetDataFormatAndOrder(eVIDEOIN_IN_UYVY, eVIDEOIN_IN_YUV422, eVIDEOIN_OUT_RGB565);
	if(priv->videopic.palette == VIDEO_PALETTE_YUV422)
		priv->pDevVin->SetDataFormatAndOrder(eVIDEOIN_IN_UYVY, eVIDEOIN_IN_YUV422, eVIDEOIN_OUT_YUV422);
...
static unsigned int vin_setbufferaddr(struct file *file)
{	
	struct video_device *dev = video_devdata(file);
	videoin_priv_t *priv = (videoin_priv_t *)dev->priv;
	UINT32 u32PacStride, u32PlaStride;
	UINT32 u32Width, u32Height;
	UINT32 u32UserStride;

	DBG_PRINTF("%s\n",__FUNCTION__);
	priv->pDevVin->GetStride(&u32PacStride, &u32PlaStride);			/* Packet Stride depend on the LCM Width */ 

	if(priv->videocrop.c.width < priv->videommap.width){
		if(priv->i32UserPlanarStride == 0)
			priv->pDevVin->SetStride(u32PacStride, priv->videocrop.c.width);
		else
			priv->pDevVin->SetStride(u32PacStride, priv->i32UserPlanarStride);
		DBG_PRINTF("Function %s - Line %d: Encode WxH = %dx%d", __FUNCTION__, __LINE__, priv->videocrop.c.width, priv->videocrop.c.height);	
		...
		if(priv->i32UserPlanarStride == 0)
			...
			priv->pDevVin->SetStride(u32PacStride, priv->i32UserPlanarStride);
		DBG_PRINTF("Function %s - Line %d: Encode WxH = %dx%d", __FUNCTION__, __LINE__, priv->videommap.width, priv->videommap.height);		
	...
	if(priv->i32UserPlanarStride == 0){/* User didn't specified stride */
		if(priv->videocrop.c.width < priv->videommap.width)
			u32UserStride = priv->videocrop.c.width;
		else
			u32UserStride = priv->videommap.width;
	}else{/* User specified stride */			
			u32UserStride = priv->i32UserPlanarStride;	
	}
	if(priv->i32UserPlanarHeight != 0)/* User didn't specified height */
		u32Height = priv->i32UserPlanarHeight;
	{
		UINT32 u32StartAddr;
		if(priv->videommap.format ==VIDEO_PALETTE_YUV422P){
			uint32_t u32YSize, u32UVSize;
			uint32_t i;
//#ifdef CONFIG_SUPPORT_EXTRA_BUFFER_DEV2
#if CONFIG_VIN_DEV2_ENCODE_BUF_NO == 3
			uint32_t u32bufnum = 3;
#else
			uint32_t u32bufnum = 2;	
#endif
			if(priv->i32UserPlanarStride!=0){
				u32YSize = priv->i32UserPlanarStride*u32Height;
				u32UVSize = u32YSize/2;
			}else{
				u32YSize = u32UserStride*u32Height;
				u32UVSize = u32YSize/2;
			}
			DBG_PRINTF("Specified encode planar YUV422\n");				
			priv->pDevVin->SetPlanarFormat(eVIDEOIN_PLANAR_YUV422);
			DBG_PRINTF("\nStride = %d\n", u32UserStride);
			DBG_PRINTF("(X, Y)= (%d, %d)\n", priv->i32planar_pos_x, priv->i32planar_pos_y);
			DBG_PRINTF("Two Y Size = 0x%x\n", u32YSize);
			DBG_PRINTF("Two U/V size = 0x%x\n", u32UVSize);
			for(i=0; i<u32bufnum; i=i+1)
			{
				priv->pFramePlanarAddr[i].u32PlaY = priv->videoIn_encode_buf[i].u32PhysAddr + priv->i32planar_pos_y*u32UserStride+priv->i32planar_pos_x;
				#if 0 //H OK. V fail
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
				priv->pFramePlanarAddr[i].u32PlaU = u32StartAddr + u32YSize + priv->i32planar_pos_y*u32UserStride+priv->i32planar_pos_x/2; 				//skip y space + offset
				#else
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
				priv->pFramePlanarAddr[i].u32PlaU = u32StartAddr + u32YSize + priv->i32planar_pos_y*u32UserStride/2+priv->i32planar_pos_x/2; 			//skip y space + offset	
				#endif
				#if 0 //H OK. V fail
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
	 			priv->pFramePlanarAddr[i].u32PlaV = u32StartAddr +u32YSize + u32UVSize+ priv->i32planar_pos_y*u32UserStride/2+priv->i32planar_pos_x/2;	//skip y space + uvsize + offset
				#else
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
	 			priv->pFramePlanarAddr[i].u32PlaV = u32StartAddr +u32YSize + u32UVSize+ priv->i32planar_pos_y*u32UserStride/2+priv->i32planar_pos_x/2;	//skip y space + uvsize + offset
				#endif
...
//#ifdef CONFIG_SUPPORT_EXTRA_BUFFER_DEV2
			...
			if(priv->i32UserPlanarStride!=0){
				u32YSize = priv->i32UserPlanarStride*u32Height;
				u32UVSize = u32YSize/4;
			}else{
				u32YSize = u32UserStride*u32Height;
				u32UVSize = u32YSize/4;
			}
			DBG_PRINTF("Specified encode planar YUV420\n");
			priv->pDevVin->SetPlanarFormat(eVIDEOIN_PLANAR_YUV420);
			DBG_PRINTF("\nStride = %d\n", u32UserStride);
			DBG_PRINTF("(X, Y)= (%d, %d)\n", priv->i32planar_pos_x, priv->i32planar_pos_y);
			DBG_PRINTF("Two Y Size = 0x%x\n", u32YSize);
			DBG_PRINTF("Two U/V size = 0x%x\n", u32UVSize);
			for(i=0; i<u32bufnum; i=i+1)
			{
				...
				#else
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
				priv->pFramePlanarAddr[i].u32PlaU = u32StartAddr + u32YSize + priv->i32planar_pos_y/2*u32UserStride/2+priv->i32planar_pos_x/2; 				//skip y space + offset
				#endif
				#if 0
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
	 			priv->pFramePlanarAddr[i].u32PlaV = u32StartAddr +u32YSize + u32UVSize+ priv->i32planar_pos_y*u32UserStride/2+priv->i32planar_pos_x/2;		//skip y space + uvsize + offset
				#else
				u32StartAddr = priv->videoIn_encode_buf[i].u32PhysAddr;
				priv->pFramePlanarAddr[i].u32PlaV = u32StartAddr + u32YSize + u32UVSize+ priv->i32planar_pos_y/2*u32UserStride/2+priv->i32planar_pos_x/2; 	//skip y space + uvsize + offset
				#endif
			}
		}
		else if(priv->videommap.format == VIDEO_PALETTE_YUV420P_MACRO)
		{/* W55FA92 support the specific format to enable H2.64 */ /* Not support planar offset */
			DBG_PRINTF("Specified encode planar \n");
			priv->pDevVin->SetPlanarFormat(eVIDEOIN_MACRO_PLANAR_YUV420);
			priv->pFramePlanarAddr[0].u32PlaY = priv->videoIn_encode_buf[0].u32PhysAddr;
			u32StartAddr = priv->pFramePlanarAddr[0].u32PlaY;
			u32StartAddr = u32StartAddr + u32Height * u32Width; //U and V for Planar YUV420 macro
											...
											void *arg)
{
	...
	{//Start Capture
		...
		{//Clear packet buffer, Black in YUV422 is Y=0x0, U=V=0x80 
			...
			unsigned int i;
			for(i=0; i<priv->i32packet_width*priv->i32packet_height*LCDBPP/8;i=i+4)
			{
	...
	{//Stop Capture
		...
		//schedule_delay(30);
		while(1){/* To avoid the stop then start capture, enginer sometimes not be enabled */  
			if(priv->pDevVin->GetOperationMode()==FALSE)
				break;
		}
	}
...
//	UINT32 u32Width, u32Height;
	...
	if((priv->videommap.format!=VIDEO_PALETTE_YUV422P)&&\
		...
		(priv->videommap.format!=VIDEO_PALETTE_YUV420P_MACRO))
	{
	...
	if(priv->videommap.width > priv->sensor_intf->u16CurImgWidth){//Encode dimension != Current sensor's width			
		if(priv->sensor_intf->change_image_resolution){
			if(priv->sensor_intf->change_image_resolution(priv, priv->videommap.width, priv->videommap.height)){
				bIsChangeBufAddr = TRUE;
				DBG_PRINTF("Change resolution successful\n");
				DBG_PRINTF("videommap resolution %d * %d\n", priv->videommap.width, priv->videommap.height);
				DBG_PRINTF("sensor resolution = %d * %d\n", priv->sensor_intf->u16CurImgWidth, priv->sensor_intf->u16CurImgHeight);					
			}
			else{
				DBG_PRINTF("Change resolution fail. may the sensor not support the specified dimension\n");
				DBG_PRINTF("videommap resolution %d * %d\n", priv->videommap.width, priv->videommap.height);
				DBG_PRINTF("sensor resolution = %d * %d\n", priv->sensor_intf->u16CurImgWidth, priv->sensor_intf->u16CurImgHeight);			
			}
		}
	...
	DBG_PRINTF("%s\n",__FUNCTION__);
	if(bIsChangeBufAddr==TRUE){
		vin_setbufferaddr(file);
	}
	/* Application want to grab the specified frame */
	priv->videoIn_encode_buf[priv->videommap.frame].u8FrameState=FRAME_GRABBING;	
										...
										void *arg)
{
	...
	if (v4l2buff.index >= CONFIG_VIN_DEV2_ENCODE_BUF_NO)
	{
												...
												void *arg)
{
	...
	DBG_PRINTF("videoin_ioctl VIDIOC_CROPCAP\n");
#ifdef CONFIG_SHARE_SENSOR
	{
		UINT16 u16Width, u16Height;
		/* From DEV1 get sensor output dimension */	
		GetVideo0SensorResoultion(&u16Width, &u16Height);
		priv->sensor_intf->u16CurImgWidth = u16Width;
		priv->sensor_intf->u16CurImgHeight = u16Height;
		
		/* 2014-01-10 */
		priv->videocropcap.defrect.left = 0;
		priv->videocropcap.defrect.top = 0;
		priv->videocropcap.defrect.width = u16Width;
		priv->videocropcap.defrect.height = u16Height;
	} 
#endif		
											...
											void *arg)
{
	...
	DBG_PRINTF("videoin_ioctl VIDIOC_S_CROP\n");
	...
	{//Cropping width < encode size, start up JPEG upscale, capturing cropping size.
		if(priv->i32UserPlanarStride == 0){
			DBG_PRINTF("Set planar stride = %d\n", priv->videocrop.c.width);			
			priv->pDevVin->SetStride(u32PacStride, priv->videocrop.c.width);
		}else{
			DBG_PRINTF("Set user specified planar stride = %d\n", priv->i32UserPlanarStride);			
			priv->pDevVin->SetStride(u32PacStride, priv->i32UserPlanarStride);
		}
		DBG_PRINTF("Function %s - Line %d: Encode WxH = %dx%d", __FUNCTION__, __LINE__, priv->videocrop.c.width, priv->videocrop.c.height);	
		priv->pDevVin->EncodePipeSize(priv->videocrop.c.height, priv->videocrop.c.width);
...
//#ifdef CONFIG_SUPPORT_EXTRA_BUFFER_DEV2
	...
	{//Crop width less than specified packet dimension ==> reset the packet start address, downscale size and packet buffer start address.
		...
		for(i=0;i<CONFIG_VIN_DEV2_PREVIEW_BUF_NO;i;i++)
		{
	...
	DBG_PRINTF("Wait a new frame done\n");
#endif
	return ret;	
}
...
//#ifdef CONFIG_MOTION_DETECTION_DEV2
static unsigned int vin_ioctl_query_diff_offset(struct file *file,
										 	unsigned int cmd,
											void *arg)
{
	...
	DBG_PRINTF("videoin_ioctl VIDIOC_G_DIFF_OFFSET\n");
	offset = priv->videoIn_md_Offset_info[1].u32MapOffsetAddr;	
	if (copy_to_user((void*)arg, (void *)(&offset), sizeof(unsigned long))) {
		ERR_PRINTF("copy_to_user error VIDIOC_G_DIFF_OFFSET\n");
		ret = -EFAULT;
	}
	return ret;		
}
static unsigned int vin_ioctl_query_diff_size(struct file *file,
									 	unsigned int cmd,
										void *arg)
{
	int ret = 0;
	...
	DBG_PRINTF("videoin_ioctl VIDIOC_G_DIFF_SIZE\n");
#if 0
	size = 	priv->videoIn_md_Offset_info[1].u32Size; 	//Allocate buffer size
#else	

	if((priv->videowin.width/8)%4==0)
		size = priv->videowin.width/8*priv->videowin.height/8;
	else
		size = (priv->videowin.width/8+(4-((priv->videowin.width/8)%4))) *priv->videowin.height/8;	
#endif
...
static unsigned int vin_ioctl_set_motion_threshold(struct file *file,
										 	unsigned int cmd,
											void *arg)
{
	struct video_device *dev = video_devdata(file);
	videoin_priv_t *priv = (videoin_priv_t *)dev->priv;
	UINT32 u32Threshold, u32DiffBuffer, u32LumBuffer;

	priv->pDevVin->GetMotionDetEx(&u32Threshold,
					&u32DiffBuffer, 
					&u32LumBuffer);	
	DBG_PRINTF("videoin_ioctl VIDIOC_S_MOTION_THRESHOLD\n");
	if (copy_from_user((void*)&u32Threshold, (void *)arg, sizeof(UINT32))) {
		ERR_PRINTF("copy_from_user error VIDIOC_S_MOTION_THRESHOLD\n");
		return -EFAULT;
	}
#ifdef CONFIG_MOTION_DETECTION_DEV1		
	priv->pDevVin->SetMotionDetEx(u32Threshold,						
					priv->videoIn_diff_buf[1].u32PhysAddr,	
					priv->videoIn_diff_buf[0].u32PhysAddr);	
								
	priv->pDevVin->SetMotionDet(TRUE,	 		
					TRUE, 						/* TRUE=8x8. FALSE = 16x16 */
					FALSE);						/* FALSE for 1 bit Threshold+7 bits diff */
	priv->pDevVin->SetMotionDetFreq(0);				/* MD each frame */
#endif
	return 0;
}
//#endif
							...
							void *arg)
{
	...
	s_packet_info.i32PipeBufNo = CONFIG_VIN_DEV2_PREVIEW_BUF_NO;	
	s_packet_info.i32PipeBufSize = priv->i32packet_width*priv->i32packet_height *LCDBPP/8;

#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
	{
		uint32_t u32CurrIndex;
		...
		s_packet_info.i32CurrBufIndex = u32CurrIndex;		
	}
#else
	s_packet_info.i32CurrPipePhyAddr = priv->videoIn_preview_buf[priv->grab_sync].u32PhysAddr;	/* if not dual sensor */
	s_packet_info.i32CurrBufIndex = priv->grab_sync;
#endif	
							...
							void *arg)
{
	...
	s_planar_info.i32PipeBufNo = CONFIG_VIN_DEV2_ENCODE_BUF_NO;	
	s_planar_info.i32PipeBufSize = priv->sensor_intf->u16MaxImgHeight * priv->sensor_intf->u16MaxImgWidth * 2;
 	
	/* need to update if supporting combination dual planar buffer */
	//s_planar_info.i32CurrPipePhyAddr = priv->videoIn_encode_buf[priv->grab_sync].u32PhysAddr;
	#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
		s_planar_info.i32CurrPipePhyAddr = priv->videoIn_encode_buf[priv->grab_sync].u32PhysAddr; 		
		s_planar_info.i32CurrBufIndex = u32SyncSharePacketBuffer;			
	#else
		s_planar_info.i32CurrPipePhyAddr = priv->videoIn_encode_buf[priv->grab_sync].u32PhysAddr;
		s_planar_info.i32CurrBufIndex = priv->grab_sync;
	#endif	
													...
													void *arg)
{
	...
	DBG_PRINTF("videoin_ioctl VIDIOC_S_MAP_BUF\n");
	if (copy_from_user((void*)&priv->i32MappingBuf, (void *)arg, sizeof(INT32))) {
		DBG_PRINTF("copy_from_user error VIDIOC_S_MAP_BUF\n");
		return -EFAULT;
	}
	if (priv->i32MappingBuf >= (CONFIG_VIN_DEV2_ENCODE_BUF_NO + CONFIG_VIN_DEV2_PREVIEW_BUF_NO) )
	{//start form 0
		DBG_PRINTF("Specified mapping buffer is wrong 0x%x error\n", priv->i32MappingBuf);
		DBG_PRINTF("Start from 0, 0 for planar bufer 0, ... packet buffer 0 start from (total planar buffer no -1 ) \n");
		return -EINVAL;
	}
	DBG_PRINTF("Specified mapping buffer = %d\n", priv->i32MappingBuf);
	return 0;
}
/*
	The function is used to combination both planar pipes for dual sensor. 
	H/V tile for dual sensor. 
*/
static unsigned int vin_ioctl_set_planar_position(struct file *file,
												 	unsigned int cmd,
													void *arg)
{
	struct video_device *dev = video_devdata(file);
	videoin_priv_t *priv = (videoin_priv_t *)dev->priv;
	S_PLANAR_POSITION sPlaPosition;

	DBG_PRINTF("videoin_ioctl VIDIOC_S_PLANAR_POSITION\n");
	memcpy(&sPlaPosition, (struct S_PLANAR_POSITION *)arg, sizeof(S_PLANAR_POSITION));
	DBG_PRINTF("Planar position (x, y) = (%d, %d)\n", sPlaPosition.i32planar_pos_x, sPlaPosition.i32planar_pos_y);
	priv->i32planar_pos_x = sPlaPosition.i32planar_pos_x;
	priv->i32planar_pos_y = sPlaPosition.i32planar_pos_y;
	return 0;
}
/*
static unsigned int vin_ioctl_query_packet_size(struct file *file,
									 	unsigned int cmd,
										void *arg)
{
	int ret = 0;
...
static unsigned int vin_ioctl_query_packet_physical_addr(struct file *file,
									 	unsigned int cmd,
										void *arg)
{
...
//#ifdef CONFIG_MOTION_DETECTION_DEV2
	vin_ioctl_query_diff_offset,
	vin_ioctl_query_diff_size,
	vin_ioctl_set_motion_threshold,
//#endif	

	vin_ioctl_query_packet_info,	/* for two pipe encode */
	vin_ioctl_query_planar_info,
	vin_ioctl_set_mapping_buffer,
	
	//vin_ioctl_query_packet_size,
	//vin_ioctl_query_packet_physical_addr, 
	vin_ioctl_set_planar_position,
};
