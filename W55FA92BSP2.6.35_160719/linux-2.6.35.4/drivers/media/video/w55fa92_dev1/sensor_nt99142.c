...
static void NTSetResolution(int index)
{
    ...
    if(s_i32SensorClk > 16000){
		...
		printk("Programming sensor init > 16MHz \n");
	...
	if(s_i32SensorClk > 16000){
		...
		printk("Programming sensor resolution > 16MHz = %d\n", index);
	}else if(s_i32SensorClk <= 16000){
...
static __s32 InitSensor(__u32 u32Sensor, void *priv)
{
	...
	printk("Init NT_99142 in port 1\n");
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, 24000, eVIDEOIN_SNR_CCIR601);	
	schedule_mdelay(10);	//It need to delay 33300 sensor clock at least
#ifdef CONFIG_W55FA92_VIDEOIN_DEV2
