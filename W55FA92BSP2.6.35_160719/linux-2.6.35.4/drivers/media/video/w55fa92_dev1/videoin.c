...
volatile unsigned int w55fa92_VIN_PAC_BUFFER;
...
module_param(videoin_dev1, int, 0);
...
#elif defined  CONFIG_SENSOR_NT99142_DEV1
...
#elif defined  CONFIG_SENSOR_NT99160_DEV1
	static __u32 u32Sensor = NT_99160;
#elif defined  CONFIG_SENSOR_NT99252_DEV1
	static __u32 u32Sensor = NT_99252;
#elif defined  CONFIG_SENSOR_NT99240_DEV1
	static __u32 u32Sensor = NT_99240;
#elif defined  CONFIG_SENSOR_NT99340_DEV1
	static __u32 u32Sensor = NT_99340;
#elif defined  CONFIG_SENSOR_HM1375_DEV1
	static __u32 u32Sensor = HM_1375;
#elif defined  CONFIG_SENSOR_HM1055_DEV1
	static __u32 u32Sensor = HM_1055;	
#elif defined  CONFIG_SENSOR_SP1628_DEV1
...
static unsigned int FramePackerAddr[CONFIG_VIN_DEV1_PREVIEW_BUF_NO]; 	/* Preview start address for packet buffer */
static planar_buf_t FramePlanarAddr[CONFIG_VIN_DEV1_ENCODE_BUF_NO]; 	/* Encode start address for planar buffer */

/* Packet Frame Buffer Pointer x3 */
#ifdef CONFIG_SUPPORT_SHARE_PLANAR_BUFFER_DEV1	/* Share planar buffer to sensor port 2 */
volatile videoIn_buf_t videoIn_share_encode_buf[CONFIG_VIN_DEV1_ENCODE_BUF_NO]={	
					{0,0,{0,0},FRAME_READY},	
					{0,0,{0,0},FRAME_READY},	
					{0,0,{0,0},FRAME_READY}		
					};		
//volatile unsigned int u32SyncSharePacketBuffer;
//EXPORT_SYMBOL(u32SyncSharePacketBuffer);
//EXPORT_SYMBOL(videoIn_share_preview_buf);
#endif
volatile static videoIn_buf_t videoIn_preview_buf[CONFIG_VIN_DEV1_PREVIEW_BUF_NO]={	
...
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1	/* Share packet buffer to sensor port 2 */
volatile videoIn_buf_t videoIn_share_preview_buf[CONFIG_VIN_DEV1_PREVIEW_BUF_NO]={	
					{0,0,{0,0},FRAME_READY},	
					{0,0,{0,0},FRAME_READY},	
					{0,0,{0,0},FRAME_READY}		
					};		
...
volatile static videoIn_buf_t videoIn_encode_buf[CONFIG_VIN_DEV1_ENCODE_BUF_NO]={	
												...
												unsigned int diff_size)
{
	...
	buf_p = w55fa92_vin_dev1_p;
	DBG_PRINTF("memory allocate for port 1");

	MPRINTF("VideoIn 1 reserved buffer size  = 0x%x\n", priv->i32AllocBufSize);
	MPRINTF("VideoIn 1 remaining buffer size = 0x%x\n", priv->i32RemainingBufSize);
	MPRINTF("VideoIn 1 reserved start buffer addr = 0x%x\n", w55fa92_vin_dev1_p);
	for(buf=0; buf<CONFIG_VIN_DEV1_ENCODE_BUF_NO; buf=buf+1){
		...
		videoIn_share_encode_buf[buf].u32PhysAddr = videoIn_encode_buf[buf].u32PhysAddr;
		videoIn_share_encode_buf[buf].u32VirtAddr = videoIn_encode_buf[buf].u32VirtAddr;				
#endif
	}
	for(buf=0; buf<CONFIG_VIN_DEV1_PREVIEW_BUF_NO; buf=buf+1){
		...
		videoIn_share_preview_buf[buf].u32PhysAddr = videoIn_preview_buf[buf].u32PhysAddr;
		videoIn_share_preview_buf[buf].u32VirtAddr = videoIn_preview_buf[buf].u32VirtAddr;				
#endif	
	}	
#ifdef CONFIG_MOTION_DETECTION_DEV1
...
//#endif 
extern unsigned int get_vin_dev1_buffer_size(void);
static int videoin_open(struct file *file)
{
	...
	if(i32OpenCount==1){
		...
		for(buf_num=0; buf_num<CONFIG_VIN_DEV1_PREVIEW_BUF_NO; buf_num=buf_num+1){
			...
			unsigned int i;
			for(i=0; i<priv->i32packet_width*priv->i32packet_height*LCDBPP/8;i=i+4){//Clear packet buffer, Black in YUV422 is Y=0x0, U=V=0x80 
	...
	//#endif
		...
		priv->i32UserPlanarStride = 0;	//New create for dual sensor H tile
	...
	else{//unnecessary to close if open fail
		spin_lock(&spin_vin_opc);
		i32OpenCount--; /* Restore */
		spin_unlock(&spin_vin_opc);
		return -EBUSY;
	}
		      ...
		      size_t count, loff_t *ppos)
{
	...
	size = priv->videowin.height * priv->videowin.width * LCDBPP/8;
	...
	*ppos += size;
	...
	priv->i32ReadFrameNum = (priv->i32ReadFrameNum+1)%CONFIG_VIN_DEV1_ENCODE_BUF_NO;
...
static int videoin_mmap (struct file *file, struct vm_area_struct *vma)
{
	...
	if(priv->i32MappingBuf < CONFIG_VIN_DEV1_ENCODE_BUF_NO)
	{
	...
	else
	{
		...
		pos = videoIn_preview_buf[(priv->i32MappingBuf-CONFIG_VIN_DEV1_ENCODE_BUF_NO)].u32PhysAddr;
	}
	...
	{
		DBG_PRINTF("Mapping packet buf = %d\n", priv->i32MappingBuf);
		pos = videoIn_preview_buf[(priv->i32MappingBuf-CONFIG_VIN_DEV1_ENCODE_BUF_NO)].u32VirtAddr;
	}
	while (size > 0)
	{
...
//#ifdef CONFIG_SUPPORT_EXTRA_BUFFER_DEV1	
	...
	if(priv->i32FrameNumber!=-1)
	{
		if (u32PlaAddr==priv->pFramePlanarAddr[0].u32PlaY){
			...
			if(priv->grab_sync != 1){
				...
				priv->pDevVin->SetBaseStartAddress(eVIDEOIN_PACKET, 0, priv->pFramePackerAddr[1]); /* Capture to packet buffer 1 */
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
				//u32SyncSharePacketBuffer = 1; /* dev2 will update to show frame 1 */
				u32SyncSharePacketBuffer = 2; /* dev2 will update to show frame 2 */
				//u32SyncSharePacketBuffer = 0; /* dev2 will update to show frame 0 */
#endif
				if(bIsVideoInEnable==TRUE)
					w55fa92_VIN_PAC_BUFFER = videoIn_preview_buf[0].u32PhysAddr;		
				//printk("A:0\n");
			...
			if(priv->grab_sync != 2){					
				...
				priv->pDevVin->SetBaseStartAddress(eVIDEOIN_PACKET, 0, priv->pFramePackerAddr[2]);	/* Capture to packet buffer 2 */
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
				//u32SyncSharePacketBuffer = 2; /* dev2 will update to show frame 2 */
				u32SyncSharePacketBuffer = 0; /* dev2 will update to show frame 0 */
				//u32SyncSharePacketBuffer = 1; /* dev2 will update to show frame 1 */
			...
			if(priv->grab_sync != 0){		
				...
				priv->pDevVin->SetBaseStartAddress(eVIDEOIN_PACKET, 0, priv->pFramePackerAddr[0]);	/* Capture to packet buffer 0 */
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
				//u32SyncSharePacketBuffer = 0; /* dev2 will update to show frame 0 */
				u32SyncSharePacketBuffer = 1; /* dev2 will update to show frame 1 */
				//u32SyncSharePacketBuffer = 2; /* dev2 will update to show frame 2 */
	...
	if(u32PacAddr == priv->pFramePackerAddr[0]){/* Show packet 0 done */		
		priv->pDevVin->SetBaseStartAddress(eVIDEOIN_PACKET, 0, priv->pFramePackerAddr[1]); /* switch to 1 */
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
		u32SyncSharePacketBuffer = 2; /* dev 2 will update to frame 2 */
#endif
		if(bIsVideoInEnable==TRUE)
			w55fa92_VIN_PAC_BUFFER = videoIn_preview_buf[0].u32PhysAddr;
	}else if(u32PacAddr == priv->pFramePackerAddr[1]){/* Show packet 1 done */		
		priv->pDevVin->SetBaseStartAddress(eVIDEOIN_PACKET, 0, priv->pFramePackerAddr[2]); /* switch to 2 */
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
		u32SyncSharePacketBuffer = 0; /* dev 2 will update to frame 0 */
#endif
		if(bIsVideoInEnable==TRUE)
			w55fa92_VIN_PAC_BUFFER = videoIn_preview_buf[1].u32PhysAddr;
	}else if(u32PacAddr == priv->pFramePackerAddr[2]){/* Show packet 2 done */	
		priv->pDevVin->SetBaseStartAddress(eVIDEOIN_PACKET, 0, priv->pFramePackerAddr[0]); /* switch to 0 */
#ifdef CONFIG_SUPPORT_SHARE_PACKET_BUFFER_DEV1
		u32SyncSharePacketBuffer = 1; /* dev 2 will update to frame 1 */
#endif
		if(bIsVideoInEnable==TRUE)
			w55fa92_VIN_PAC_BUFFER = videoIn_preview_buf[2].u32PhysAddr;
	}
	...
	if(uframe%200==0){
		uint32_t ubuf;
		printk("Frame = %d\n", uframe);
		for(ubuf=0; ubuf<CONFIG_VIN_DEV1_ENCODE_BUF_NO; ubuf=ubuf+1)
		{
...
//#endif
	...
	priv->i32AllocBufSize = get_vin_dev1_buffer_size();	//DEV 1
	...
	priv->i32packet_width = CONFIG_VIN_DEV1_PREVIEW_WIDTH;
	priv->i32packet_height = CONFIG_VIN_DEV1_PREVIEW_HEIGHT;
...
#endif	
	strcpy((char *)&priv->videocap.name, "W55FA92 Camera dev1");
	...
	if( videoin_dmamalloc_packet_phy((u32VinPreBufSize+PAGE_SIZE)) <= 0)
		return -EPIPE;
#endif
	DBG_PRINTF("Encode buf no.= %d\n", CONFIG_VIN_DEV1_ENCODE_BUF_NO);		
	DBG_PRINTF("Preview buf no.= %d\n", CONFIG_VIN_DEV1_PREVIEW_BUF_NO);
	...
	for(i=CONFIG_VIN_DEV1_ENCODE_BUF_NO; i<(CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO); i++)
	{
		priv->videombuf.offsets[i] = 0 + (videoIn_preview_buf[(i-CONFIG_VIN_DEV1_ENCODE_BUF_NO)].u32VirtAddr - videoIn_encode_buf[0].u32VirtAddr);	
		MPRINTF("mbuf[%d].offset = 0x%x\n", i, priv->videombuf.offsets[i]);
	}	

#ifdef CONFIG_MOTION_DETECTION_DEV1
	for(i=CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO; 
		i<CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO+VIDEOIN_DIFF_BUFFER_NUM; i=i+1){
		priv->videombuf.offsets[i] = 0 + (priv->videoIn_diff_buf[i-(CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO)].u32VirtAddr -\
										videoIn_encode_buf[0].u32VirtAddr);
		MPRINTF("mbuf[%d].offset = 0x%x\n", i, priv->videombuf.offsets[i]);
		priv->videoIn_md_Offset_info[i-(CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO)].u32MapOffsetAddr = priv->videombuf.offsets[i];
		priv->videoIn_md_Offset_info[i-(CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO)].u32Size = u32VinDiffBufSize;		
	}
#else
	priv->videoIn_md_Offset_info[i-(CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO)].u32MapOffsetAddr = 0;
	priv->videoIn_md_Offset_info[i-(CONFIG_VIN_DEV1_ENCODE_BUF_NO+CONFIG_VIN_DEV1_PREVIEW_BUF_NO)].u32Size = 0;	
#endif

	priv->videombuf.frames = CONFIG_VIN_DEV1_ENCODE_BUF_NO; 
	/* The frames only report encode buffer no. Other buffers need through VIDIOC_G_PACKET_INFO and VIDIOC_G_PLANAR_INFO */
#ifdef CONFIG_MOTION_DETECTION_DEV1
	priv->videombuf.size = (u32VinEncBufSize + PAGE_SIZE)*CONFIG_VIN_DEV1_ENCODE_BUF_NO +\
							(u32VinPreBufSize+ PAGE_SIZE)*CONFIG_VIN_DEV1_PREVIEW_BUF_NO +\
							(u32VinDiffBufSize+ PAGE_SIZE)*VIDEOIN_DIFF_BUFFER_NUM;
#else
	priv->videombuf.size = (u32VinEncBufSize + PAGE_SIZE)*CONFIG_VIN_DEV1_ENCODE_BUF_NO + (u32VinPreBufSize+ PAGE_SIZE)*CONFIG_VIN_DEV1_PREVIEW_BUF_NO;
#endif
	...
	if (!request_mem_region((unsigned long)W55FA92_VA_VIDEOIN, W55FA92_SZ_VIDEOIN, "w55fa92-videoin-dev1")){
	...
	ret = request_irq(IRQ_CAP, irq_handler,  IRQF_DISABLED | IRQF_SHARED, "w55fa92-videoin-dev1", priv);
	if (ret) {
...
#endif
	printk("Video capture device 1 initialize successful\n");
	return ret;
