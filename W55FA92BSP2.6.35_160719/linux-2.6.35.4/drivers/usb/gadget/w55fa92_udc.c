/*
 * linux/drivers/usb/gadget/w55fa92_udc.c
 *
 * Nuvoton w55fa92 MCU on-chip full speed USB device controllers
 *
...
#include <mach/w55fa92_usbd.h>
#include <mach/map.h>
//#include <mach/regs-gcr.h>
#include "w55fa92_udc.h"
...
static const char       gadget_name [] = "w55fa92-udc";
...
static struct w55fa92_udc controller;

static void udc_isr_rst(struct w55fa92_udc *dev);
static void udc_isr_dma(struct w55fa92_udc *dev);
static void udc_isr_ctrl_pkt(struct w55fa92_udc *dev);
static void udc_isr_update_dev(struct w55fa92_udc *dev);
static u32 udc_transfer(struct w55fa92_ep *ep, u8* buf, size_t size, u32 mode);


static void nuke (struct w55fa92_udc *udc, struct w55fa92_ep *ep)
{

        while (!list_empty (&ep->queue)) {
                struct w55fa92_request  *req;
                req = list_entry (ep->queue.next, struct w55fa92_request, queue);
...
static void done(struct w55fa92_ep *ep, struct w55fa92_request *req, int status)
{
        struct w55fa92_udc *udc = &controller;
...
static void start_write(struct w55fa92_ep *ep, u8* buf,u32 length)
{
        struct w55fa92_udc *dev = ep->dev;
        u32 volatile reg;
...
static void start_read(struct w55fa92_ep *ep, u8* buf, u32 length)
{
        struct w55fa92_udc	*dev = ep->dev;
...
static inline void clear_ep_state (struct w55fa92_udc *dev)
{
        unsigned i;
 ...
 */
static inline int
write_packet(struct w55fa92_ep *ep, struct w55fa92_request *req)
{
        struct w55fa92_udc *udc = &controller;
        unsigned	len;
        u8		*buf;
...
static int write_fifo(struct w55fa92_ep *ep, struct w55fa92_request *req)
{
        u32 len;
...
static inline int read_packet(struct w55fa92_ep *ep,u8 *buf,
                              struct w55fa92_request *req, u16 cnt)
{
        struct w55fa92_udc *udc = &controller;
...
static int read_fifo(struct w55fa92_ep *ep, struct w55fa92_request *req, u16 cnt)
{
        u8		*buf;
...
void paser_irq_stat(int irq, struct w55fa92_udc *dev)
{
	u32 volatile reg;
...
void paser_irq_cep(int irq, struct w55fa92_udc *dev, u32 IrqSt)
{
        struct w55fa92_ep	*ep = &dev->ep[0];
        struct w55fa92_request	*req;
        int		is_last=1;
        if (list_empty(&ep->queue)) {
                req = 0;
        ...
        case CEP_IN_TOK:
                ...
                if (dev->setup_ret < 0) { // == -EOPNOTSUPP)
                        ...
                        __raw_writel(CEP_ZEROLEN, REG_USBD_CEP_CTRL_STAT);
                        __raw_writel(0x400, REG_USBD_CEP_IRQ_ENB);		//enb sts completion int
                }
...
void paser_irq_nep(int irq, struct w55fa92_ep *ep, u32 IrqSt)
{
        struct w55fa92_udc *dev = ep->dev;
        struct w55fa92_request	*req;
        int i;
...
void paser_irq_nepint(int irq, struct w55fa92_ep *ep, u32 IrqSt)
{
        struct w55fa92_udc *dev = ep->dev;
        struct w55fa92_request	*req;
        __raw_writel(irq, REG_USBD_EPA_IRQ_STAT + 0x28*(ep->index-1));
...
/*
 *      w55fa92_udc_irq - interrupt handler
 */
static irqreturn_t w55fa92_udc_irq(int irq, void *_dev)
{
        struct w55fa92_udc *dev;
        struct w55fa92_ep *ep;
        u32 volatile IrqStL, IrqEnL;
        u32 volatile  IrqSt, IrqEn;
        int i=0, j;
	
        dev=(struct w55fa92_udc *)(_dev);
	g_usbd_access++;
...
//0-3F for Ctrl pipe
s32 get_sram_base(struct w55fa92_udc	*dev, u32 max)
{
        int i, cnt = 1, j;
        s32 start, end;

        for (i = 1; i < W55FA92_ENDPOINTS; i++) {
                struct w55fa92_ep *ep = &dev->ep[i];
 ...
 * 	w55fa92_ep_enable
 */
static int w55fa92_ep_enable (struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
        struct w55fa92_udc	*dev;
        struct w55fa92_ep	*ep;
        u32			max, tmp;
        ...
        u32			int_en_reg;
        s32 sram_addr;
        ep = container_of (_ep, struct w55fa92_ep, ep);
 ...
 * w55fa92_ep_disable
 */
static int w55fa92_ep_disable (struct usb_ep *_ep)
{
        struct w55fa92_ep *ep = container_of(_ep, struct w55fa92_ep, ep);
        unsigned long	flags;
        if (!_ep || !ep->desc) {
                return -EINVAL;
        }
...
/*
 * w55fa92_alloc_request
 */
static struct usb_request *
w55fa92_alloc_request (struct usb_ep *_ep, gfp_t mem_flags) {
        struct w55fa92_ep	*ep;
        struct w55fa92_request	*req;

        //printk("w55fa92_alloc_request(ep=%p,flags=%d) ", _ep, mem_flags);
        ep = container_of (_ep, struct w55fa92_ep, ep);
        if (!_ep)
                return 0;
...
/*
 * w55fa92_free_request
 */
static void
w55fa92_free_request (struct usb_ep *_ep, struct usb_request *_req)
{
        struct w55fa92_ep	*ep;
        struct w55fa92_request	*req;

        //printk("w55fa92_free_request(ep=%p,req=%p)\n", _ep, _req);

        ep = container_of (_ep, struct w55fa92_ep, ep);
        if (!ep || !_req || (!ep->desc && _ep->name != ep0name))
                return;

        req = container_of (_req, struct w55fa92_request, req);
 ...
 */
static int
w55fa92_queue(struct usb_ep *_ep, struct usb_request *_req, gfp_t gfp_flags)
{
        struct w55fa92_request	*req;
        struct w55fa92_ep	*ep;
        struct w55fa92_udc	*dev;
        ...
        if (unlikely (!_req || !_req->complete || !_req->buf
                ...
                if (!_req) {
                        printk("w55fa92_queue: 1 X X X\n");
                } else {
                        printk("w55fa92_queue: 0 %01d %01d %01d\n",!_req->complete,!_req->buf, !list_empty(&req->queue));
                }
 ...
 * 	w55fa92_dequeue
 */
static int w55fa92_dequeue (struct usb_ep *_ep, struct usb_request *_req)
{
        struct w55fa92_ep	*ep;
        struct w55fa92_udc	*udc = &controller;
        int			retval = -EINVAL;
        ...
        printk("w55fa92_dequeue(ep=%p,req=%p)\n", _ep, _req);
        ...
        if (!_ep || !_req)
                return retval;
        ep = container_of (_ep, struct w55fa92_ep, ep);
        udc = container_of (ep->gadget, struct w55fa92_udc, gadget);
 ...
 * w55fa92_set_halt
 */
static int w55fa92_set_halt (struct usb_ep *_ep, int value)
{
        printk("set halt\n");
        return 0;
}
...
static const struct usb_ep_ops w55fa92_ep_ops = {
        .enable         = w55fa92_ep_enable,
        .disable        = w55fa92_ep_disable,

        .alloc_request  = w55fa92_alloc_request,
        .free_request   = w55fa92_free_request,

        .queue          = w55fa92_queue,
        .dequeue        = w55fa92_dequeue,

        .set_halt       = w55fa92_set_halt,
};

/*
 * 	w55fa92_g_get_frame
 */
static int w55fa92_g_get_frame (struct usb_gadget *_gadget)
{
        int tmp;
 ...
 * 	w55fa92_wakeup
 */
static int w55fa92_wakeup (struct usb_gadget *_gadget)
{
        return 0;
}

/*
 * 	w55fa92_set_selfpowered
 */
static int w55fa92_set_selfpowered (struct usb_gadget *_gadget, int value)
{
...
static const struct usb_gadget_ops w55fa92_ops = {
        .get_frame          = w55fa92_g_get_frame,
        .wakeup             = w55fa92_wakeup,
        .set_selfpowered    = w55fa92_set_selfpowered,
};
...
void udc_isr_update_dev(struct w55fa92_udc *dev)
{
...
static void USB_Init(struct w55fa92_udc *dev)
{
        int	j;
 ...
 */
static int /*__init*/ w55fa92_udc_probe(struct platform_device *pdev)
{
        struct w55fa92_udc *udc = &controller;
	...
	printk("w55fa92_udc_probe 20140401\n");
        udc->pdev = pdev;
        udc->gadget.dev.parent = &pdev->dev;
        udc->gadget.dev.dma_mask = pdev->dev.dma_mask;

	__raw_writel(__raw_readl(REG_CLKDIV2) & ~0x0060F0E0, REG_CLKDIV2);
 ...
 //	__raw_writel(__raw_readl(REG_AHBCLK) | USBD_CKE, REG_AHBCLK);
  	...
  	//__raw_writel(__raw_readl((REG_USBD_PHY_CTL) | (0x20 | Phy_suspend), REG_USBD_PHY_CTL);    // offset 0x704
        ...
        error = request_irq(udc->irq, w55fa92_udc_irq,
 ...
 * 	w55fa92_udc_remove
 */
static int __exit w55fa92_udc_remove(struct platform_device *pdev)
{
        struct w55fa92_udc *udc = platform_get_drvdata (pdev);
        dev_dbg(&pdev->dev, "%s()\n", __func__);
...
#ifdef CONFIG_PM
static int w55fa92_udc_suspend (struct platform_device *pdev, pm_message_t state)
{
        // TODO:
        return 0;
}

static int w55fa92_udc_resume (struct platform_device *pdev)
{
        // TODO:
        return 0;
}
#else
#define w55fa92_udc_suspend     NULL
#define w55fa92_udc_resume      NULL
#endif

static struct platform_driver udc_driver = {
        .probe		= w55fa92_udc_probe,
        .remove		= __exit_p(w55fa92_udc_remove),
        .suspend	= w55fa92_udc_suspend,
        .resume		= w55fa92_udc_resume,
        .driver		= {
                .owner	= THIS_MODULE,
                .name	= (char *) "w55fa92-usbgadget",
