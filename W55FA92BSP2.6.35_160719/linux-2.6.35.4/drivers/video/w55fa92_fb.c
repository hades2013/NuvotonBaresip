...
//		printk("IO PAN command received !!!\n");
//		printk("g_var_xoffset = 0x%x !!! \n", g_var_xoffset);
//		printk("g_var_yoffset = 0x%x !!! \n", g_var_yoffset);		
...
//				printk("reg[0x%x] = 0x%x !!! \n", ii*4, inl(REG_LCM_LCDCCtl+ii*4));										
			...
			outl(inl(REG_GPIOD_DOUT) | 0x02, REG_GPIOD_DOUT);		// backlight ON (for Nuvoton FA92 demo board only)
		#endif			
	...
	//		_vpost_int = 0;
			...
			outl(inl(REG_GPDFUN) & ~MF_GPD1, REG_GPDFUN);	
			outl(inl(REG_GPIOD_OMD) | 0x02, REG_GPIOD_OMD);				
			outl(inl(REG_GPIOD_DOUT) & ~0x02, REG_GPIOD_DOUT);		// backlight OFF (for Nuvoton FA92 demo board only)
		#endif			
	...
	//		clock_div = w55fa92_apll_clock / 9000;
			clock_div &= 0xFF;
			clock_div --;
...
//	#ifdef CONFIG_W55FA92_TV_LCD_SWITCH & (!CONFIG_W55FA92_TV_QVGA)
...
//	outl(CONFIG_FRAME_BUFFER_ADDR, REG_LCM_FSADDR);
	...
	//w55fa92_edma_request(VDMA_CH,"w55fa92-VPOST");
        //w55fa92_edma_enable(VDMA_CH);
...
//	fbinfo->var.yres_virtual  	= mach_info->yres.defval;
   ...
   if ( clk_edma == NULL )
   {
	...
	//ret = request_irq(irq, w55fa92fb_irq, 0, pdev->name, info);
...
#define w55fa92fb_resume  NULL
	...
	//int __devinit w55fa92fb_init(void)
	...
	//	return platform_driver_probe(&w55fa92fb_driver, w55fa92fb_probe);
	}
#else
	static struct platform_driver w55fa92fb_driver = {
	//	.probe		= w55fa92fb_probe,
		.remove		= w55fa92fb_remove,
	//	.suspend	= w55fa92fb_suspend,
	//	.resume		= w55fa92fb_resume,
		.driver		= {
			.name	= "w55fa92-lcd",
